...
const MyProfile = () => {
  ...
  return userData && (
    <div className='max-w-lg flex flex-col gap-2 text-sm mt-5'>
      ...
      <div>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 text-neutral-700'>
          <p className='font-medium'>Gender:</p>
          {
            ...
            {
              isEdit ? <input className='max-w-28 bg-gray-200' type="date" onChange={(e) => setUserData(prev => ({ ...prev, dob: e.target.value}))} value={userData.dob}/>
      ...
      <div className='mt-10'>
        {
          isEdit ? <button className='border border-primary hover:text-white px-8 py-2 rounded-full hover:bg-primary transition-all duration-500' onClick={updateUserProfileData}>Save Information</button>
          : <button className='border border-primary hover:text-white px-8 py-2 rounded-full hover:bg-primary transition-all duration-500' onClick={() => setIsEdit(true)}>Edit</button>
        }
      </div>
    </div>
  )
